...
(function($) {
  ...
  $.gomoku = function(el, options) {
    ...
    var defaults = {
      ...
      game = this,
      board = [],
      board_size, cells, is_player_turn = false,
          ...
          for (j = 4; j--;) {
            ...
            for (k = (!board[i] ? 5 : 1); k--;) {
              ...
              if (line.length == 7 && undefined !== type) {
                ...
                for (l = 5; l--;)
                  if (board[line[l + 1]] == type) total_cells++;
                ...
                score = [
                  ...
                  [10, 64],
                  [256, 256]
                ][consecutive_cells >= total_cells ? Math.min(consecutive_cells, 5) - 1 : total_cells - 1][consecutive_cells >= total_cells ? (empty_sides ? empty_sides - 1 : 0) : 0];
